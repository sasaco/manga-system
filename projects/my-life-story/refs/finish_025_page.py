"""Finish episode 025 with separated Krita balloon and lettering layers."""
...
def main() -> None:
    ...
    lines = [
        line.strip()
        for line in (PROJECT / "lettering" / "025.txt").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    if lines != ["業務委託として", "うちに来い。"]:
        raise ValueError("lettering/025.txt does not match the approved two-line dialogue")

    balloon = Image.new("RGBA", (WIDTH * SCALE, HEIGHT * SCALE))
    balloon_draw = ImageDraw.Draw(balloon)
    bubble = uneven_oval((70, 67, 700, 365), 0.8)
    balloon_draw.polygon([scaled(point) for point in bubble], fill="white")
    wobbly_line(balloon_draw, bubble, width=3.7, closed=True)
    tail = [(544, 350), (561, 478), (614, 344)]
    ...
    for y, line in zip((186, 274), lines):
        lettering_draw.text(
            scaled((384, y)),
            line,
            font=font,
    ...
    balloon.save(REFS / "025-balloon.png")
    lettering.save(REFS / "025-lettering.png")

    preview = Image.new("RGBA", (WIDTH, HEIGHT), "white")
    preview = Image.alpha_composite(
        preview, Image.open(REFS / "025-color.png").convert("RGBA")
    )
    gray = ImageOps.grayscale(Image.open(REFS / "025-control.png"))
    ...
    preview.save(REFS / "025-preview.png")

    ora = PROJECT / "pages" / ".025-finished.ora"
    prepare_page(
        ROOT / "templates" / "krita" / settings["page_template"],
        PROJECT / "panels" / "selected" / "025.png",
        ora,
        line_art=REFS / "025-control.png",
        color_art=REFS / "025-color.png",
    )
