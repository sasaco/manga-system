...
# Balloon: upper left-center, roomy
bubble = oval_points((95, 75, 470, 560), phase=0.8)
balloon_draw.polygon([px(p) for p in bubble], fill=WHITE)
wobble(bubble, width=4.0, jitter=0.75, closed=True, target=balloon_draw)

# Tail towards apprentice at right-center
tail = [(425, 480), (485, 525), (450, 450)]
...
comic_font = ImageFont.truetype(font_path, 34 * SCALE, layout_engine=ImageFont.Layout.BASIC)

def vertical_text(text, x, y, spacing=42):
    for index, char in enumerate(text):
        lettering_draw.text(px((x, y + index * spacing)), char, font=comic_font, fill=INK, anchor='mm')

vertical_text('一円でも', 375, 185, spacing=46)
vertical_text('お金をもらったら', 295, 145, spacing=38)
vertical_text('プロなんだ', 215, 180, spacing=44)

here = Path('projects/my-life-story/refs')
balloon_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '009-balloon.png')
lettering_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '009-lettering.png')

# Preview with line and color
line_img = Image.open(here / '009-control.png').convert('RGBA')
color_img = Image.open(here / '009-color.png').convert('RGBA')
...
preview.save(here / '009-preview.png')
print('Created 009 preview successfully')
