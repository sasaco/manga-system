"""Finish episode 024 with separated Krita balloon and lettering layers."""
...
def main() -> None:
    ...
    lines = [
        line.strip()
        for line in (PROJECT / "lettering" / "024.txt").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    if lines != ["ごめんなさい。", "間に合わないです。"]:
        raise ValueError("lettering/024.txt does not match the approved two-line dialogue")

    balloon = Image.new("RGBA", (WIDTH * SCALE, HEIGHT * SCALE))
    balloon_draw = ImageDraw.Draw(balloon)
    bubble = uneven_oval((86, 68, 682, 358), 0.8)
    balloon_draw.polygon([scaled(point) for point in bubble], fill="white")
    wobbly_line(balloon_draw, bubble, width=3.7, closed=True)
    tail = [(276, 340), (246, 476), (336, 345)]
    ...
    for y, line in zip((188, 270), lines):
        lettering_draw.text(
            scaled((384, y)),
            line,
            font=font,
    ...
    balloon.save(REFS / "024-balloon.png")
    lettering.save(REFS / "024-lettering.png")

    preview = Image.new("RGBA", (WIDTH, HEIGHT), "white")
    preview = Image.alpha_composite(
        preview, Image.open(REFS / "024-color.png").convert("RGBA")
    )
    gray = ImageOps.grayscale(Image.open(REFS / "024-control.png"))
    ...
    preview.save(REFS / "024-preview.png")

    ora = PROJECT / "pages" / ".024-finished.ora"
    prepare_page(
        ROOT / "templates" / "krita" / settings["page_template"],
        PROJECT / "panels" / "selected" / "024.png",
        ora,
        line_art=REFS / "024-control.png",
        color_art=REFS / "024-color.png",
    )
