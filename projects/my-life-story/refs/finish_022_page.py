"""Finish episode 022 using separate Krita artwork, balloon, and text layers."""
...
lettering_draw.text(px((387, 216)), "はい、担当です。", font=font, fill=INK, anchor="mm")

balloon = balloon.resize((W, H), Image.Resampling.LANCZOS)
lettering = lettering.resize((W, H), Image.Resampling.LANCZOS)
balloon.save(HERE / "022-balloon.png")
lettering.save(HERE / "022-lettering.png")

preview = Image.new("RGBA", (W, H), "white")
preview = Image.alpha_composite(preview, Image.open(HERE / "022-color.png").convert("RGBA"))
gray = ImageOps.grayscale(Image.open(HERE / "022-control.png"))
...
preview.save(HERE / "022-preview.png")

settings = json.loads((P / "project.json").read_text(encoding="utf-8"))
ora = P / "pages/.022-finished.ora"
prepare_page(
    ROOT / "templates/krita" / settings["page_template"],
    P / "panels/selected/022.png",
    ora,
    line_art=HERE / "022-control.png",
    color_art=HERE / "022-color.png",
)
