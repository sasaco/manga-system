...
# ── Balloon: upper right area ──
bubble = oval_points((140, 50, 620, 270), phase=0.8)
balloon_draw.polygon([px(p) for p in bubble], fill=WHITE)
wobble(bubble, width=4.2, jitter=0.75, closed=True, target=balloon_draw)

# Tail: pointing toward protagonist's head
tail = [(360, 260), (385, 330), (415, 255)]
...
bold_font = ImageFont.truetype(bold_path, 27 * SCALE, layout_engine=ImageFont.Layout.BASIC)
...
# 2 columns, right-to-left
vertical_text('〆切が厳しいから…！', 465, 95, spacing=33, font=bold_font)
vertical_text('作業を代行してるんだ', 275, 100, spacing=34, font=reg_font)

here = Path('projects/my-life-story/refs')
balloon_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '015-balloon.png')
lettering_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '015-lettering.png')

line_img = Image.open(here / '015-control.png').convert('RGBA')
color_img = Image.open(here / '015-color.png').convert('RGBA')
...
preview.save(here / '015-preview.png')
print('Updated 015-preview.png')
