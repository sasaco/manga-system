...
# ── Balloon: positioned in upper-left ──
bubble = oval_points((105, 60, 525, 360), phase=0.8)
balloon_draw.polygon([px(p) for p in bubble], fill=WHITE)
wobble(bubble, width=4.2, jitter=0.75, closed=True, target=balloon_draw)

# Tail: slightly curved, pointing down-right toward protagonist
tail = [(260, 345), (320, 440), (335, 340)]
...
vertical_text('どんな球でも…！', 425, 110, spacing=35, font=reg_font)
vertical_text('絶対に後ろへ', 320, 115, spacing=35, font=bold_font)
vertical_text('逸らさない…っ！', 215, 110, spacing=35, font=bold_font)

here = Path('projects/my-life-story/refs')
balloon_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '012-balloon.png')
lettering_layer.resize((WIDTH, HEIGHT), Image.Resampling.LANCZOS).save(here / '012-lettering.png')

line_img = Image.open(here / '012-control.png').convert('RGBA')
color_img = Image.open(here / '012-color.png').convert('RGBA')
...
preview.save(here / '012-preview.png')
print('Updated 012-preview.png')
