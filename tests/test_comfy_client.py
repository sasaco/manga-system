...
SPEC = importlib.util.spec_from_file_location(
    "comfy_client", ROOT / "scripts" / "comfy_client.py"
)
...
class WorkflowTests(unittest.TestCase):
    def test_build_workflow_replaces_runtime_values(self):
        template = json.loads(
            (ROOT / "templates" / "comfy" / "panel_txt2img_api.json").read_text(
                encoding="utf-8"
            )
        )
        result = MODULE.build_workflow(
            template,
            checkpoint="manga.safetensors",
            prompt="hero",
            negative="text",
            seed=42,
            width=640,
    ...
    def test_build_workflow_replaces_controlnet_values(self):
        template = json.loads(
            (ROOT / "templates" / "comfy" / "panel_controlnet_api.json").read_text(
                encoding="utf-8"
            )
        )
        result = MODULE.build_workflow(
            template,
            checkpoint="manga.safetensors",
            prompt="two stick figures, no text",
            ...
            sampler="dpmpp_2m",
            scheduler="karras",
            prefix="manga/control",
            control_image="002-control.png",
            controlnet="scribble.safetensors",
            control_strength=1.15,
        )
        self.assertEqual(result["8"]["inputs"]["image"], "002-control.png")
        self.assertEqual(
            result["10"]["inputs"]["control_net_name"], "scribble.safetensors"
        )
...
class ComfyClientTests(unittest.TestCase):
    def test_control_upload_name_is_content_addressed(self):
        with tempfile.TemporaryDirectory() as directory:
            left_dir = Path(directory) / "left"
            right_dir = Path(directory) / "right"
            left_dir.mkdir()
            right_dir.mkdir()
            left = left_dir / "guide.png"
            right = right_dir / "guide.png"
            left.write_bytes(b"first")
            right.write_bytes(b"second")

            self.assertNotEqual(
                MODULE.control_upload_name(left), MODULE.control_upload_name(right)
            )
            self.assertEqual(
                MODULE.control_upload_name(left), MODULE.control_upload_name(left)
            )
